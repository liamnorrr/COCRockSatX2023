...
void eth_begin(bool debug) {
  ...
  if (debug) Serial.println("Ethernet setup begin.");
  ...
  if (debug) Serial.println("Begin configuration.");
  delay(100);
  ...
  if (debug) Serial.println("Ethernet setup completed.");
}

void eth_send_message(bool debug, String& message) {
  if (debug) Serial.println("Begin send message.");
  if (eth_availableForWrite() > 0) {
    SERIAL_ETH.print(message);
  } else if (debug) SERIAL_ETH.println("No room for write.");
  if (debug) Serial.println("Send message complete.");
}

String eth_receive_message(bool debug) {
  if (debug) Serial.println("Begin receive message.");
  String message = "TEST";
  if (eth_available() == 0) {
    if (debug) Serial.println("Nothing to read.");
    message = "";
  } else {
    message = SERIAL_ETH.readString();
  }
  if (debug) Serial.println("Recieve message complete.");
  return message;
}
...
void eth_debug() {
  ...
  SERIAL_ETH.write(0x57);
  SERIAL_ETH.write(0xAB);
  SERIAL_ETH.write(0x62);  // Get Mask
  SERIAL_ETH.readBytes(bytes, 4);
  Serial.print("Mask: ");
  Serial.print(bytes[0], DEC);
  Serial.print("-");
  ...
  SERIAL_ETH.write(0x57);
  SERIAL_ETH.write(0xAB);
  SERIAL_ETH.write(0x64);  // Get Local Port
  ...
  SERIAL_ETH.write(0x57);
  SERIAL_ETH.write(0xAB);
  SERIAL_ETH.write(0x65);  // Get Target IP
  SERIAL_ETH.readBytes(bytes, 4);
  ...
  SERIAL_ETH.write(0x57);
  SERIAL_ETH.write(0xAB);
  SERIAL_ETH.write(0x66);  // Get Target Port
  SERIAL_ETH.readBytes(bytes, 2);
  ...
  SERIAL_ETH.write(0x57);
  SERIAL_ETH.write(0xAB);
  SERIAL_ETH.write(0x71);  // Get Target Port
  SERIAL_ETH.readBytes(bytes, 4);
  Serial.print("Baud: ");
